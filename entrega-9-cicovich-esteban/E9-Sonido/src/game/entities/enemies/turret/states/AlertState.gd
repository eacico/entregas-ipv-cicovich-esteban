...
onready var alert_sfx:AudioStream = load("res://assets/sound/sfx/enemies/Scream_Robotic.wav")
onready var attack_sfx:AudioStream = load("res://assets/sound/sfx/enemies/20_orc_special_atk.wav")

func enter() -> void:
	# Luego triggereamos un disparo y reproducimos la animación
	fire()
	character.play_sound(alert_sfx)
...
func fire() -> void:
	...
	character.play_sound(attack_sfx)
